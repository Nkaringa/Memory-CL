...
# ---------------------------------------------------------------------------
# Federated identities — OIDC/OAuth2 provider ↔ user linkage
# ---------------------------------------------------------------------------
@runtime_checkable
...
# ---------------------------------------------------------------------------
# Teams + team memberships — sub-org groupings with membership
# ---------------------------------------------------------------------------
@runtime_checkable
class TeamRepository(Protocol):
    async def ensure_schema(self) -> None: ...
    async def create_team(self, *, team_id: str, org_id: str, name: str, slug: str) -> "TeamRow": ...
...
# ---------------------------------------------------------------------------
# Repo grants — per-repo access control (user or team subject)
# ---------------------------------------------------------------------------
@runtime_checkable
class RepoGrantRepository(Protocol):
    async def ensure_schema(self) -> None: ...
    async def grant(self, *, id: str, org_id: str, repo_id: str, subject_type: str, subject_id: str, access: str) -> "RepoGrantRow": ...
    async def get(self, id: str) -> "RepoGrantRow | None": ...
    async def list_for_repo(self, *, repo_id: str) -> "list[RepoGrantRow]": ...
    async def list_for_subjects(self, *, org_id: str, user_id: str, team_ids: list[str]) -> "list[RepoGrantRow]": ...
    async def revoke(self, id: str) -> None: ...
    async def delete_for_repo(self, repo_id: str) -> None: ...


# ---------------------------------------------------------------------------
# Invitations — token-hash-keyed org membership invitations
# ---------------------------------------------------------------------------
@runtime_checkable
class InvitationRepository(Protocol):
    async def ensure_schema(self) -> None: ...
    async def create(self, *, id: str, org_id: str, email: str, role: str, token_hash: str, invited_by: str, expires_at: datetime) -> "InvitationRow": ...
